...
class PaymentIntent extends Stancer\Tests\atoum
{
            ...
            ->then
                ...
                ->assert('Aliases')
                    ->variable($this->newTestedInstance->get_amount())
                        ->isNull
                    ->object($this->testedInstance->set_amount($amount))
                        ->isTestedInstance
                    ...
                    ->variable($this->newTestedInstance->amount)
                        ->isNull
                    ->given($this->testedInstance->amount = $amount)
        ...
        $this
            ...
            ->then
                ...
                ->assert('Aliases')
                    ...
                    ->then
                        ->variable($this->testedInstance->get_capture())
                            ->isNull
                        ->object($this->testedInstance->set_capture(true))
                            ->isTestedInstance
                        ->boolean($this->testedInstance->get_capture())
                            ->isTrue

                        ->object($this->testedInstance->set_capture(false))
                            ->isTestedInstance
                        ->boolean($this->testedInstance->get_capture())
                            ->isFalse
        ;
        ...
        $this
            ...
            ->then
                ...
                ->assert('Aliases')
                    ...
                    ->variable($this->newTestedInstance->get_card())
                        ->isNull
                    ->object($this->testedInstance->set_card($card))
                        ->isTestedInstance
                    ->string($this->testedInstance->get_card()->getId())
                        ->isIdenticalTo($card->getId())

                    ->variable($this->newTestedInstance->card)
                        ->isNull
                    ->given($this->testedInstance->card = $card)
                        ->string($this->testedInstance->card->getId())
                            ->isIdenticalTo($card->getId())
        ;
    }
    ...
    public function testListPayments()
    {
        $this
            ...
            ->and($config = $this->mockConfig($client, Stancer\Enum\ApiVersion::VERSION_2))
            ->and($options = $this->mockRequestOptions($config))
            ->and($this->newTestedInstance('pi_' . $this->getRandomString(27)))
            ->then
                ->assert('Invalid limit')
                    ->exception(function () {
                        $this->testedInstance->payments(['limit' => 0]);
                    })
                        ->isInstanceOf(Stancer\Exceptions\InvalidSearchLimitException::class)
                        ->message
                            ->isIdenticalTo('Limit must be between 1 and 100.')
                    ...
                    ->exception(function () {
                        $this->testedInstance->payments(['limit' => 101]);
                    })
                        ->isInstanceOf(Stancer\Exceptions\InvalidSearchLimitException::class)
                        ->message
                            ->isIdenticalTo('Limit must be between 1 and 100.')
                    ...
                    ->exception(function () {
                        $this->testedInstance->payments(['limit' => uniqid()]);
                    })
                        ->isInstanceOf(Stancer\Exceptions\InvalidSearchLimitException::class)
                        ->message
                            ->isIdenticalTo('Limit must be between 1 and 100.')
                ...
                ->assert('Invalid start')
                    ->exception(function () {
                        $this->testedInstance->payments(['start' => -1]);
                    })
                        ->isInstanceOf(Stancer\Exceptions\InvalidSearchStartException::class)
                        ->message
                            ->isIdenticalTo('Start must be a positive integer.')
                    ...
                    ->exception(function () {
                        $this->testedInstance->payments(['start' => uniqid()]);
                    })
                        ->isInstanceOf(Stancer\Exceptions\InvalidSearchStartException::class)
                        ->message
                            ->isIdenticalTo('Start must be a positive integer.')
                ...
                ->assert('No terms')
                    ...
                    ->exception(function () {
                        $this->testedInstance->payments(['foo' => 'bar']);
                    })
                        ->isInstanceOf(Stancer\Exceptions\InvalidSearchFilterException::class)
                        ->message
                            ->isIdenticalTo('Invalid search filters.')
                ...
                ->assert('Invalid created filter')
                    ->exception(function () {
                        $this->testedInstance->payments(['created' => time() + 100]);
                    })
                        ->isInstanceOf(Stancer\Exceptions\InvalidSearchCreationFilterException::class)
                        ->message
                            ->isIdenticalTo('Created must be in the past.')
                    ...
                    ->exception(function () {
                        ...
                        $this->testedInstance->payments(['created' => $date]);
                    })
                        ->isInstanceOf(Stancer\Exceptions\InvalidSearchCreationFilterException::class)
                        ->message
                            ->isIdenticalTo('Created must be in the past.')
                    ...
                    ->exception(function () {
                        $this->testedInstance->payments(['created' => 0]);
                    })
                        ->isInstanceOf(Stancer\Exceptions\InvalidSearchCreationFilterException::class)
                        ->message
                            ->isIdenticalTo('Created must be a positive integer, a DateTime object or a DatePeriod object.')
                    ...
                    ->exception(function () {
                        $this->testedInstance->payments(['created' => uniqid()]);
                    })
                        ->isInstanceOf(Stancer\Exceptions\InvalidSearchCreationFilterException::class)
                        ->message
                            ->isIdenticalTo('Created must be a positive integer, a DateTime object or a DatePeriod object.')
                ...
                ->assert('Make request')
                    ...
                    ->then
                        ->generator($gen = $this->testedInstance->payments($terms1))
                            ->yields
                                ->object
                ...
                ->assert('Make request, alias: listPayments')
                    ->given($client = new mock\Stancer\Http\Client())
                    ->and($response = $this->mockJsonResponse('payment', 'list'))
                    ->and($this->calling($client)->request = $response)
                    ->and($config = $this->mockConfig($client, Stancer\Enum\ApiVersion::VERSION_2))
                ...
                ->assert('Empty response, alias: listPayments')
                    ->given($body = [
                        'payments' => [],
                        'range' => [
                            'has_more' => false,
                ...
                ->assert('Invalid response, alias: listPayments')
                    ->given($this->calling($response)->getBody = new Stancer\Http\Stream(''))
    ...
    public function testMetadata()
    {
        $this
            ...
            ->assert('Aliases')
                ->given($this->newTestedInstance->metadata = $asString)
                    ->string($this->testedInstance->metadata)
                        ->isIdenticalTo($asString)

                ->given($this->newTestedInstance->metadata = $asInteger)
                    ->integer($this->testedInstance->Metadata)
                        ->isIdenticalTo($asInteger)

                ->given($this->newTestedInstance->metadata = $asArray)
                    ->array($this->testedInstance->metadata)
                        ->hasSize(2)

                ->given($this->newTestedInstance->metadata = $asDict)
                    ->array($this->testedInstance->metadata)
                        ->hasSize(2)

                ->given($this->newTestedInstance->metadata = $asJsonObject)
                    ->object($this->testedInstance->metadata)
                        ->isIdenticalTo($asJsonObject)

                ->given($this->newTestedInstance->metadata = $asStringable)
                    ->object($this->testedInstance->metadata)
                        ->isIdenticalTo($asStringable)
        ...
        $this
            ...
            ->then
                ...
                ->assert('Aliases')
                    ...
                    ->given($this->newTestedInstance->set_order_id($orderId))
                    ...
                    ->given($this->newTestedInstance->orderId = $orderId)
                    ...
                    ->given($this->newTestedInstance->order_id = $orderId)
            ...
            ->then
                ...
                ->assert('Aliases')
                    ->variable($this->newTestedInstance->get_payment())
                        ->isNull
                    ->given($this->testedInstance->hydrate(['payment' => $payment]))
            ...
            ->then
                ...
                ->assert('Aliases')
                    ...
                    ->variable($this->newTestedInstance->returnUrl)
                        ->isNull
                    ->given($this->testedInstance->returnUrl = $https)
                        ->string($this->testedInstance->returnUrl)
                            ->isIdenticalTo($https)

                    ->variable($this->newTestedInstance->return_url)
                        ->isNull
                    ->given($this->testedInstance->return_url = $https)
        ...
        $this
            ...
            ->then
                ->assert('Flat send card')
                    ...
                    ->and($cardOptions = $this->mockRequestOptions($config, ['body' => json_encode($card)]))
                    ->and($this->calling($client)->request[1] = $this->mockResponse(json_encode(['id' => $cardId])))
                ...
                ->assert('Flat send customer')
                    ...
                    ->given($customerLocation = $customer->getUri())
                    ->and($customerOptions = $this->mockRequestOptions($config, ['body' => json_encode($customer)]))
                    ->and($this->calling($client)->request[2] = $this->mockResponse(json_encode(['id' => $customerId])))
                    ->then
                ...
                ->assert('Flat send both')
                    ...
                    ->and($options = $this->mockRequestOptions($config, ['body' => json_encode($this->testedInstance)]))
                    ->and($this->calling($client)->request = $this->mockJsonResponse('payment_intents', 'intent'))
                    ->then
                        ->object($this->testedInstance->send())
                        ->mock($client)
                            ->call('request')
    ...
    public function testSend_flatenedCard()
    {
        $this
            ...
            ->given($location = $this->testedInstance->getUri())
            ->and($cardLocation = $card->getUri())
            ->and($cardOptions = $this->mockRequestOptions($config, ['body' => json_encode($card)]))
            ->and($body = $this->testedInstance->jsonSerialize())
            ->and($this->calling($client)->request[0] = $this->mockResponse(json_encode(['id' => $cardId])))
            ->then
    ...
    public function testSend_flatenedCustomer()
    {
        $this
            ...
            ->given($location = $this->testedInstance->getUri())
            ->and($customerLocation = $customer->getUri())
            ->and($customerOptions = $this->mockRequestOptions($config, ['body' => json_encode($customer)]))
            ->and($body = $this->testedInstance->jsonSerialize())
            ->and($this->calling($client)->request[0] = $this->mockResponse(json_encode(['id' => $customerId])))
            ->then
    ...
    public function testSend_flatenedSepa()
    {
        $this
            ...
            ->and($sepaLocation = $sepa->getUri())
            ->and($sepaOptions = $this->mockRequestOptions($config, ['body' => json_encode($sepa)]))
            ->and($body = $this->testedInstance->jsonSerialize())
            ->and($this->calling($client)->request = $this->mockResponse(json_encode(['id' => $sepaId])))
            ->then
        ...
        $this
            ...
            ->then
                ...
                ->assert('Aliases')
                    ...
                    ->variable($this->newTestedInstance->get_sepa())
                        ->isNull
                    ->object($this->testedInstance->set_sepa($sepa))
                        ->isTestedInstance
                    ->object($this->testedInstance->get_sepa())
                        ->isIdenticalTo($sepa)

                    ->variable($this->newTestedInstance->sepa)
                        ->isNull
                    ->given($this->testedInstance->sepa = $sepa)
    ...
    public function testStatus()
    {
        $this
            ...
            ->then
                ...
                ->assert('Aliases')
                    ->variable($this->newTestedInstance->get_status())
                        ->isNull
                    ->object($this->testedInstance->hydrate(['status' => $status]))
                        ->isTestedInstance
                    ->object($this->testedInstance->get_status())
                        ->isIdenticalTo($status)

                    ->variable($this->newTestedInstance->status)
                        ->isNull
                    ->object($this->testedInstance->hydrate(['status' => $status]))
                        ->isTestedInstance
                    ->object($this->testedInstance->status)
                        ->isIdenticalTo($status)
        ;
    }
        ...
        $this
            ...
            ->then
                ...
                ->assert('Aliases')
                    ...
                    ->variable($this->newTestedInstance->get_three_ds())
                        ->isNull
                    ->object($this->testedInstance->set_three_ds($threeDS))
                        ->isTestedInstance
                    ->object($this->testedInstance->get_three_ds())
                        ->isIdenticalTo($threeDS)

                    ->variable($this->newTestedInstance->threeDS)
                        ->isNull
                    ->given($this->testedInstance->threeDS = $threeDS)
                    ->object($this->testedInstance->threeDS)
                        ->isIdenticalTo($threeDS)

                    ->variable($this->newTestedInstance->get_threeds())
                        ->isNull
                    ->object($this->testedInstance->set_threeds($threeDS))
                        ->isTestedInstance
                    ->object($this->testedInstance->get_threeds())
                        ->isIdenticalTo($threeDS)
                    ...
                    ->variable($this->newTestedInstance->three_ds)
                        ->isNull
                    ->given($this->testedInstance->three_ds = $threeDS)
                    ->object($this->testedInstance->three_ds)
                        ->isIdenticalTo($threeDS)

                    ->variable($this->newTestedInstance->get3DS())
                        ->isNull
                    ->object($this->testedInstance->set3DS($threeDS))
                        ->isTestedInstance
                    ->object($this->testedInstance->get3DS())
                        ->isIdenticalTo($threeDS)

                    ->variable($this->newTestedInstance->get_3ds())
                        ->isNull
                    ->object($this->testedInstance->set_3ds($threeDS))
                        ->isTestedInstance
                    ->object($this->testedInstance->get_3ds())
                        ->isIdenticalTo($threeDS)

                    ->variable($this->newTestedInstance->{'3DS'})
                        ->isNull
                    ->given($this->testedInstance->{'3DS'} = $threeDS)
                    ->object($this->testedInstance->{'3DS'})
                        ->isIdenticalTo($threeDS)
    ...
    public function testUrl()
    {
        $this
            ...
            ->then
                ...
                ->assert('Aliases')
                    ->variable($this->newTestedInstance->get_url())
                        ->isNull
                    ->given($this->testedInstance->hydrate(['url' => $url]))
                    ->string($this->testedInstance->get_url())
                        ->isIdenticalTo($url)

                    ->variable($this->newTestedInstance->url)
                        ->isNull
                    ->given($this->testedInstance->hydrate(['url' => $url]))
                    ->string($this->testedInstance->url)
                        ->isIdenticalTo($url)

                    ->variable($this->newTestedInstance->get_payment_page_url())
                        ->isNull
                    ->given($this->testedInstance->hydrate(['url' => $url]))
                    ->string($this->testedInstance->get_payment_page_url())
                        ->isIdenticalTo($url)

                    ->variable($this->newTestedInstance->getPaymentPageUrl())
                        ->isNull
                    ->given($this->testedInstance->hydrate(['url' => $url]))
                    ->string($this->testedInstance->getPaymentPageUrl())
                        ->isIdenticalTo($url)
